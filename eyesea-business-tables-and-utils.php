<?php
   /*
   Plugin Name: Eye Sea Business Tables and Utils
   Plugin URI: https://eyesea.studio
   description: A plugin to work with the eyesea-business theme
   Version: 1.0
   Author: JC Icaro
   Author URI: https://jc.icaro.com.au
   License: GPL2
   */

include_once( plugin_dir_path( __FILE__ ) . 'server-includes/ess_custom_post_types.php' );
include_once( plugin_dir_path( __FILE__ ) . 'server-includes/ess_classes.php' );
include_once( plugin_dir_path( __FILE__ ) . 'server-includes/ess_rest.php' );
include_once( plugin_dir_path( __FILE__ ) . 'server-includes/ess_fixes.php' );
// include_once( plugin_dir_path( __FILE__ ) . 'server-includes/ess_components.php' );

?>